...
public class Messenger {
         ...
         while(keepon) {
            ...
            if (authorisedUser != null) {
              ...
              while(usermenu) {
                ...
                System.out.println("\t6. View Chat");
                System.out.println("\t.........................");
                ...
                switch (readChoice()){
                   ...
                   case 5: int temp = DeleteAccount(esql);
                            if(temp == 1) usermenu = false;
                            break;
                   case 6: ViewChats(esql);
                            chatview = true; break;
                ...
                if(chatview) {
                    ...
                    switch(readChoice()){
                        case 1: validchat = SelectChat(esql); break;
   ...
   public static void AddToContact(Messenger esql){
      ...
      try{
        ...
        if(userNum > 0){
            ...
            String error = String.format("Can't find selected user '%s'",user);
   ...
   public static void ReadNotifications(Messenger esql){
     ...
     public static void ViewChats(Messenger esql){
      // Your code goes here.
      try{
          String displayChats = String.format("------DISPLAYING '%s' CHATS", SUPER_USER);
          System.out.println(displayChats);
   ...
   public static int SelectChat(Messenger esql){
      // Your code goes here.
        System.out.print("SELECT id FROM CHAT IDs: ");
       try {
            String chat_selected = in.readLine();
            String query = String.format("SELECT DISTINCT msg_timestamp, sender_login, M.msg_text FROM message M, chat_list C WHERE M.chat_id = '%s' AND C.member = '%s'", chat_selected, SUPER_USER);
            int validChat = esql.executeQueryAndPrintResult(query);
            
            if(validchat)
            {
                int initial_sender = (SELECT FROM chat WHERE init_sender = '%s' AND chat_id 's', SUPER_USER,  
                chatview = true;
                while(chatview)
                {
                    ...
                    switch(readChoice()){
                        case 1: 
                            AddMember(esql, chat_selected); break;
                        case 2: 
                            DeleteMember(esql, chat_selected); break;
                        case 3: ReplytoChat(esql, chat_selected); break;
                        case 4: 
                            DeleteWholeChat(esql, chat_selected); break;
                        case 9: chatview = false; break;
                        default : System.out.println("Unrecognized choice!"); break; 
                    }
                }
                ...
                System.out.println("Chat_id you selected is not valid");
            }
         }catch (Exception e) {
            System.out.println("Your input is invalid!");
            return 0;
   ...
   public static void AddMember(Messenger esql, String chat_id){
       System.out.print("SELECT user TO ADD TO CHAT: ");
       try{
           String user_selected = in.readLine();
           String query = String.format("INSERT INTO chat_list(chat_id, member) VALUES ('%s', '%s')", chat_id, user_selected);
       }catch (Exception e) {
